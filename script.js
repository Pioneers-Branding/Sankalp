$(document).ready(function () {
  ...
  $("#hero-widget-form, #detailed-booking-form").submit(function (event) {
    ...
    var name = form.find('#book-name').val();
    var phone = form.find('#book-phone').val();
    var dept = form.find('#book-dept').val() || form.find('#book-dept-hidden').val() || '';
    var doc = form.find('#book-doc').val() || form.find('#book-doc-hidden').val() || '';
    var date = form.find('#book-date').val() || '';
    var msg = form.find('#book-msg').val() || '';
    
    var fullMessage = "Specialty: " + dept + "\nDoctor: " + doc + "\nPreferred Date: " + date + "\nConcerns: " + msg;
    
    $.ajax({
      url: 'https://app.formester.com/forms/UvGLKUCJe/submissions',
      method: 'POST',
      data: {
        name: name,
        phone: phone,
        subject: 'Appointment Request - ' + dept,
        message: fullMessage
      },
      success: function () {
      ...
      error: function () {
        // Fallback demo success in case of network block or missing API endpoint
        alert(
