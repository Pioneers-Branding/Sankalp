...
document.getElementById('dd-booking-form').addEventListener('submit',function(e){
    ...
    const specialty = this.querySelectorAll('select')[0].value;
    const doctor = this.querySelectorAll('select')[1].value;
    ...
    formData.append('subject', 'Appointment Request - ' + specialty);
    formData.append('message', 'Specialty: ' + specialty + '\nDoctor: ' + doctor);
    
    fetch('https://app.formester.com/forms/UvGLKUCJe/submissions', {
        method: 'POST',
        body: formData
    })
    .then(response => {
